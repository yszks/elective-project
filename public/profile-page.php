...
if (file_exists($config_path)) {
    ...
    // Redirect to login or show a critical error to the user
...
if (!isset($_SESSION['id'])) { // Now correctly checking for 'id'
...
if ($stmt === false) {
    ...
    die("Database error. Please try again later. (Error: Prepare User Data)");
}
...
if ($result->num_rows !== 1) {
    ...
    session_unset();
    session_destroy();
    error_log("User with ID " . $id . " not found in database, clearing session.");
    header("Location: login-page.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['user_img'])) {
    ...
    if (in_array($imageExtension, $allowed)) {
        ...
        $uploadDir = __DIR__ . '/assets/uploads/'; // Path relative to profile-page.php

        // Create directory if it doesn't exist
        if (!is_dir($uploadDir)) {
            if (!mkdir($uploadDir, 0755, true)) {
                $_SESSION['profile_update'] = "Failed to create upload directory. Check server permissions.";
        ...
        if (move_uploaded_file($imageTmpPath, $uploadPath)) {
            // Update database with new image filename using prepared statement
            ...
            if ($stmt === false) {
                ...
                if ($stmt->execute()) {
                    ...
                    error_log("Database execute error for image update (user ID: " . $id . "): " . $stmt->error);
            ...
            error_log("Failed to move uploaded file from " . $imageTmpPath . " to " . $uploadPath . " for user ID: " . $id);
        }
    } else {
...
<head>
    ...
    <style>
        ...
        .profile-container {
            ...
            border-radius: 8px;
            text-align: center;
            position: relative; /* For fixed button positioning */
        }
        #title-profile img {
            max-width: 100%;
            height: auto;
        ...
        .profile-container .fixed-button {
            position: fixed;
            top: 20px;
            left: 20px;
            z-index: 100;
...
<body>
    <div class="profile-container">
        ...
        <a href="index.php" target="_self"><button class="fixed-button">← Return to Homepage</button></a>
        ...
        <div class="info-cont">
            <form method="POST" enctype="multipart/form-data" id="uploadForm">
                <input type="hidden" name="profile_update_trigger" value="1">
